  ...
  <div class="distribution-detail">
      ...
      <div class="spqd">
        ...
        <div class="spqd-list">
          ...
          <div class="fl-left good-name">
            ...
            <!-- 规格属性 -->
            <p class="p2 c-666" v-show="detail.skuValuesTitleEng">
              Specifications：{{ detail.skuValuesTitleEng }}
            </p>
            <!-- 出库时间 -->
            <p class="p2 c-666" v-show="detail.stockOutTime">
              Delivery time:{{ detail.stockOutTime }}
            </p>
            <p class="p2 c-666">reporting time: {{ detail.reportTime }}</p>
            <p class="p2 c-666" style="color: red">
              <span v-show="detail.sysNum"
                >System inventory:{{ detail.sysNum }};</span
              >
              <span v-show="detail.reportNum"
                >Report inventory:{{ detail.reportNum }};</span
              >
              <span v-show="detail.differNum"
                >Differential inventory:{{ detail.differNum }}</span
              >
            </p>
            <span class="status-kuang" v-if="detail.outStocktype">{{
              orderStatus(detail.outStocktype, "statusList")
            }}</span>
          </div>
        </div>
      </div>
    ...
    <div class="order-time">
      ...
      <div v-if="positionList.length > 0">
        ...
        >
          ...
          <div class="fs-22 c-666">
            ...
            <div>
              System available inventory:{{ position.canUseNum }}; reported
              inventory:{{ position.reportUseNum }}
            </div>
          </div>
        </div>
        <div
          class="error-tips"
          :style="{ color: detail.remarksState == 2 ? '#333' : '#D23535' }"
        >
      ...
      <div class="time-item" v-else>
        The current system available inventory is 0, unable to display specific
