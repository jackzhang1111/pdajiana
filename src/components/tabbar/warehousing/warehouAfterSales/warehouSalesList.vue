...
export default {
    ...
    mounted() {
        ...
        if(this.active == 3){
            this.formData.pdaorderStatus = 4
        }else if(this.active == 0){
            this.formData.pdaorderStatus = -1
        }else{
            this.formData.pdaorderStatus = this.active
        }
    },  
    watch: {
    ...
    methods: {
        ...
        onClick(index) {
            if(index == 3){
                ...
                this.formData.pdaorderStatus = index
            }
            sessionStorage.setItem("activeIndex", index);
            this.refreshOrder()
        },
        search(value){
            this.$router.push({name:'warehouSalesSearch',query:{expressNo:value}})
        },
        //售后列表
