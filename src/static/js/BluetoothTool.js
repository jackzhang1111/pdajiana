...
export default{
	...
	BluetoothTool:function () {
		...
		function turnOnBluetooth() {
			if(btAdapter == null) {
				shortToast("No Bluetooth");
				return;
			}
			if(!btAdapter.isEnabled()) {
				if(activity == null) {
					shortToast("Activity not obtained");
					return;
				} else {
				...
				shortToast("The Bluetooth was opened");
			}
		}
		...
		function turnOffBluetooth() {
			...
			if(btAdapter != null && btAdapter.isEnabled()) {
				btAdapter.disable();
				shortToast("Successfully opened the Bluetooth");
			} else {
				shortToast("The Bluetooth was closed");
			}
		}
		...
		function getPairedDevices() {
			...
			if(btAdapter != null && btAdapter.isEnabled()) {
				...
				shortToast("The Bluetooth didn’t open");
			}
		...
		function connDevice(address) {
			...
			try {
				...
				console.error(e);
				shortToast("Failed to connect, failed to get socket!");
				return false;
			}
			try {
				invoke(btSocket, "connect");
				readData(); //读数据
				shortToast("Successful connection");
			} catch(e) {
				console.error(e);
				shortToast("Failed connection");
				try {
					btSocket.close();
					btSocket = null;
				} catch(e1) {
		...
		function disConnDevice() {
			...
			state.readThreadState = false;
			shortToast("Successful disconnection");
		}
		...
		function readData() {
			if(!btSocket) {
				shortToast("Please connect the Bluetooth device first");
				return false;
			}
			try {
				...
				console.error(e);
				shortToast("Failed to create I / O stream!");
				closeBtSocket();
				return false;
			}
			let setTimeCount = 0;
		...
		function sendData(str) {
			if(!btOutStream) {
				shortToast("Failed to create output stream!");
				return;
			}
			//console.log(str);
